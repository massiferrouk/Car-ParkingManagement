...
public class PanelCRUDEtage extends JPanel{
	...
	private void initUI() {
		...
		cbSearchFields = new JComboBox<>( new String[] {ALL_FIELDS, "Numero", "Description"} );
		...
		tableModel = new MyTableModel("N°", "Numero", "Description");
	...
	private void editItem() {
		...
		Etage item = (Etage)row.get(row.size()-1);
		...
		boolean validated = formPanel.showInDialog(  SwingUtilities.getWindowAncestor(this), item );
		if (validated) {
			Etage otherItem = formPanel.getItem();
	...
	private String generateInfo(Etage item) {
		String info = "<html>";
...
//		ToolTipManager.sharedInstance().mouseMoved(mouseEvent);
	...
	private void exportPDF() {
		File file = UIUtils.createAndPrepareEmptyPDFFile(this, "list_etages");
		
		if (file == null) {
			return;
		}
		
//		List<Etage> listItems = DAOEtage.getList( this.getCondtion() );
//		ManualPDFCreator.PDFEtage.createPDFList(file, listItems);
	}
	
	private void exportExcel() {
//		File file = UIUtils.createAndPrepareEmptyExcelFile(this, "list_etages", UIUtils.PathResourceExcel.EXCEL_ETAGES);
//		
//		if (file == null) {
//			return;
//		}
...
//			
//			List<Etage> listItems = DAOEtage.getListInstances( this.getCondtion() );
//			
//			int baseRow = 12;
//			
...
//		}
	...
	private String getCondtion() {
		...
		if (fields.equals(ALL_FIELDS)) {
			condition = " CONCAT(COALESCE(`numero`,''), ' ', COALESCE(`description`,'')) like '%"+txtFilter+"%' ";
		}
		else if (fields.equals("Numero")) {
			condition = " `numero` like '%"+txtFilter+"%' ";
		}
		else if (fields.equals("Description")) {
			condition = " `description` like '%"+txtFilter+"%' ";
		}
	...
	public void updateData(){
		...
		int count = (int)DAOEtage.getCountInTable ( this.getCondtion() );
		
		guiPagination.setNbTotalItems(count);
		
		listItems = DAOEtage.getList( this.getCondtion()+" "+this.getLimits() );
		
		int num=guiPagination.getFirst()+1;
		for (Etage item : listItems) {
	...
	private Vector<Object> getRow(Etage item){
		return getRow( tableModel.tableRows.size()+1, item );
	}
	
	private Vector<Object> getRow(int num, Etage item){
	...
	public static class FormPanel extends JPanel{
		...
		private void initUI() {
			...
			tfNumero = GUIFieldFactory.createNumericField(4, 0);
			tfDescription = GUIFieldFactory.createSimpleTextField();
		...
		private void layoutUI() {
			...
			layout.setHorizontalGroup( layout.createParallelGroup()
					.addGroup(layout.createSequentialGroup()
							...
							.addGroup(layout.createParallelGroup()
									...
									.addComponent(tfDescription, 300, 300, 300)
							)
			...
			layout.setVerticalGroup( layout.createSequentialGroup()
					...
					.addGap(3)
					.addGroup(layout.createParallelGroup()
							.addComponent(lDescription, 25, 25, 25)
							.addComponent(tfDescription, 25, 25, 25)
					)
					.addGap(25, 25, Short.MAX_VALUE)
		...
		private void bValiderActionPerformed() {
			java.lang.Integer numero = 0;
			java.lang.String description = tfDescription.getText().trim();
		 ...
		 */
		public void setItem(Etage item) {
			this.item = item;
			fillFields();
		}
		...
		public boolean showInDialog(Window window, Etage item){
			this.setItem( item );
